...
function dntheme_get_the_archive_title() {
    ...
    if ( is_category() ) {
        $termz = get_queried_object();
        $image = get_field('icon',$termz);

        if($image){
            $title = '<h1 class="page__header__title">'.wp_get_attachment_image( $image, array('45', '45') ) . single_term_title( '', false ) . '</h1>';
        }else{
            $title = '<h1 class="page__header__title">'. single_term_title( '', false ) . '</h1>';
        }
        ...
        $title = '<h1 class="page-title page__header__title"><img src="'.get_theme_file_uri('assets/img/ic-tv.png').'" alt="">' . post_type_archive_title( '', false ) . '</h1>';
    } elseif ( is_tax() ) {
...
function get_post_loadmore() {
    ...
    <input type="hidden" class="js-loadmore-stt" value="<?php echo $end  ?>">
    <?php
    die();
}
